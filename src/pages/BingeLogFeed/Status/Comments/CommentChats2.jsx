                        ...
                        <article className = {styles.commentContent}>
                            <p>{comment.text}</p> 
                                {comment.replies?.length === 0 && 
                                    <img onClick = {() => replyHandler(`comment-${index}`)}src={CommentIcon} width="40px"/>
                                }
                                ...
                                <article className = {styles.replyContent}>
                                    <p>{reply.text}</p>
                                      {replyIndex === comment.replies.length - 1 &&
                                        <img onClick = {() => replyHandler(`reply-${index}-${replyIndex}`)} src={CommentIcon} width="40px"/>
                                        }
                                    {displayReply ===`reply-${index}-${replyIndex}` &&  
                                        <ReplyComment status={status} threadId={threadId} replyAuthor = {reply.authorUserName} commentId={comment.commentId}/>
                                    }
                                </article> 
                            </div>
                        )}
