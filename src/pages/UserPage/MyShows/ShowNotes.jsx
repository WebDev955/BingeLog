...
function ShowNotes({ epTitle, showTitle }) {
  ...
  async function checkWatchedEp(epTitle, showTitle) {
    const epExists = watchedEps.some(
      (ep) => ep.epName === epTitle && ep.showName === showTitle,
    );

    let updatedWatchedEpList = null;

    if (epExists) {
      updatedWatchedEpList = watchedEps.filter(
        (ep) => !(ep.epName === epTitle && ep.showName === showTitle),
      );
    } else {
      updatedWatchedEpList = [
        ...watchedEps,
    ...
    <main className={styles.showNotesWrapper}>
      <div className={styles.noteBttnsDiv}>
        {areNotesOpen === epTitle ? (
          <Bttn
            className={styles.saveNotesBttn}
            onClick={() => saveNotes(epTitle)}
          >
            Save Notes
          </Bttn>
        ) : (
          <Bttn className={styles.editBttn} onClick={() => openNotes(epTitle)}>
            Edit Notes
          </Bttn>
        )}
        {areNotesOpen !== epTitle && isViewingNotes === epTitle ? (
          ...
          <Bttn
            className={styles.viewNotesBttn}
            onClick={() => displayNotes(epTitle)}
          >
            View Notes
          </Bttn>
        )}
        <Input
          ...
          type="checkbox"
          value={epTitle}
          checked={watchedEps.some(
            (watchedEp) =>
              watchedEp.epName === epTitle && watchedEp.showName === showTitle,
          )}
