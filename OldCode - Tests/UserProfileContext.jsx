  ...
  async function saveShow(show) {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/users/${userAcct.userAccount.id}`,
        {
          method: "PATCH",
      ...
      localStorage.setItem(
        "user",
        JSON.stringify({ ...userAcct.userAccount, myShows: updatedShows }),
      );
      // sync local state
    } catch (err) {
      console.error(err);
...
***********************/
  ...
  const [eNotes, setEpNotes] = useState({});
  ...
  function saveNotes(id) {
    ...
    localStorage.setItem(
      "CharNotes: " + id,
      JSON.stringify(characterNotes[id] || ""),
    );
